...
- legacyCreateRootFromDOMContainer这个函数的返回结果:是个new ReactRoot的实例对象reactRoot
...
```
到了这里出现一个有意思的东西ReactWork, 从名字来看类似一个job的东西吧(一个猜测)，从这句work.then(callback)来看，还是个Promise。
我们先看看ReactWork是什么：
```js
